...
def hangman(secret_word):
    ...
    * After each guess, you should display to the user the
    ...
    print("I'm thinking of a",len(secret_word),"letter word")
    letters_guessed = []

    while i < n_guesses:
      print(get_guessed_word(secret_word, letters_guessed))
      print("you have ",n_guesses-i,"guesses remaining")
...
def match_with_gaps(my_word, other_word):
    ...
    result = True
    x = "".join(my_word.split())

    if len(x) == len(other_word):
      i = 0
      for char in x:
        if char == "_":
          if other_word[i] in x:
            result = False
        elif not other_word[i] == char:
          result = False
        i += 1
    else:
      result = False
